...
public class ListarCargo extends HttpServlet {
    ...
    try{  
        ...
        if(tipo == 2){
            request.getRequestDispatcher("administrador/listarcargo.jsp").forward(request, response);
        }else if(tipo == 1){
            request.getRequestDispatcher("profissional/listarcargo.jsp").forward(request, response);
        }
    }catch (Exception ex){
